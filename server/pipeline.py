import base64
import numpy as np
import cv2
import torch
from ultralytics import YOLO

from PIL import Image
import torchvision.transforms as transforms
from torchvision.models import resnet50, ResNet50_Weights


# The pipeline process for server queries is as follows
# The user queries the server with a base64 string of a coded image, and a command
# This file deals only with the base64 string image

# The base64 string is converted into a cv2 image object
# The image is then searched through using YOLO-FACE to find a bounding box of a face
# The image is then cropped to that face
# The cropped image is then standardized to fit some size (experimentation to be done
#   regarding best way to fit to a size)
# The standardized image is then passed through a image to vector model designed to
#   encode meaning of the image within the vector


yolo_model = None
vector_model = None
vec_preprocess = None

def load_models():
    global yolo_model
    yolo_model = YOLO("model/yolov11m-face.pt")

    global vector_model
    full_vector_model = resnet50(weights=ResNet50_Weights.IMAGENET1K_V1)
    full_vector_model.eval()
    vector_model = torch.nn.Sequential(*list(full_vector_model.children())[:-1])

    global vec_preprocess
    vec_preprocess = transforms.Compose([
        transforms.Resize((224, 224)),
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])


def base64_to_image(base64_str : str):
    img_data = base64.b64decode(base64_str)
    img_array = np.frombuffer(img_data, np.uint8)
    return cv2.imdecode(img_array, cv2.IMREAD_COLOR)

def find_face(image):
    global yolo_model
    if yolo_model == None:
        load_models()
    
    """
    # for now assume only 1 face
    box = results[0].boxes[0]
    x1, y1, x2, y2 = map(int, box.xyxy[0])

    face_only = image[y1:y2, x1:x2]
    return face_only
    """
    try:
        return yolo_model(image)
    except:
        return None

W = 224
H = 224

def scale_img(image, yolo_result, targ_size=(W, H)):
    # For now assume 1 face found
    box = yolo_result[0].boxes[0]
    x1, y1, x2, y2 = map(int, box.xyxy[0])
    # center = ((x1 + x2) // 2, (y1 + y2) // 2)

    center = ((y1 + y2) / (2 * image.shape[0]), (x1 + x2) / (2 * image.shape[1]))

    currW = x2 - x1
    currH = y2 - y1
    targW, targH = targ_size
    
    scalar = 0
    if currW > currH:
        scalar = targW / currW
    else:
        scalar = targH / currH
    
    newW, newH = image.shape[:2]

    newW *= scalar
    newH *= scalar
    
    resized = cv2.resize(image, (int(newH), int(newW)), interpolation=cv2.INTER_AREA)

    return resized, center

def crop_image(image, center, size=(W, H)):
    cenY, cenX = center
    
    cenX = int(cenX * image.shape[1])
    cenY = int(cenY * image.shape[0])

    width, height = size
    x1 = cenX - width // 2
    x2 = cenX + width // 2

    y1 = cenY - height // 2
    y2 = cenY + height // 2

    return image[y1:y2, x1:x2]

def convert_to_vec(image):
    if vector_model == None:
        load_models()

    pil_image = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
    input_tensor = vec_preprocess(pil_image).unsqueeze(0)
    with torch.no_grad():
        img_vector = vector_model(input_tensor)
    return img_vector.reshape(-1).numpy()

"""
def scale_img(image, target_size=(224, 224)):
    targ_width, targ_height = target_size
    curr_width, curr_height = image.shape[:2]

    aspect_ratio = curr_width / curr_height
    target_ratio = targ_width / targ_height

    new_width = 0
    new_height = 0

    if aspect_ratio > target_ratio:
        new_width = targ_width
        new_height = int(new_width / aspect_ratio)
    else:
        new_height = targ_height
        new_width = int(new_height * aspect_ratio)
    
    resized = cv2.resize(image, (new_width, new_height), interpolation=cv2.INTER_AREA)

    x_off = (targ_width - new_width) // 2
    y_off = (targ_height - new_height) // 2

    final_img = np.zeros((targ_width, targ_height, 3), dtype=np.uint8)
    final_img[y_off:y_off + new_height, x_off:x_off + new_width] = resized

    return final_img

"""

def show_img(image):
    print(image.shape)
    cv2.imshow("Result", image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()


def pipeline(base64_str : str):
    full_img = base64_to_image(base64_str)
    yolo_result = find_face(full_img)
    if len(yolo_result[0].boxes) == 0:
        raise ValueError("Could not find face.")
        
    
    scaled, center = scale_img(full_img, yolo_result)
    cropped = crop_image(scaled, center)
    vector = convert_to_vec(cropped)

    return vector.tolist(), cropped

"""
test = base64_to_image("")

# cv2.imshow("Decoded Image", test)
# cv2.waitKey(0)
# cv2.destroyAllWindows()

bounds = find_face(test)
scaled, center = scale_img(test, bounds)
cropped = crop_image(scaled, center)

vector = convert_to_vec(cropped)

print(vector.shape)
print(vector)

# show_img(cropped)
"""


# test = ""
# vector, processed = pipeline(test)
# show_img(base64_to_image(test))
# show_img(processed)
